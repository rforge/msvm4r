dlrm.filter <- function(y,x,A,Q,R,ws,Sigma,nt=nrow(x),nx=ncol(x),lt=1,bt=1,et=nt) {
  ...
  for(case in 1:lt) {
    ...
    for(i in bt[case]:(et[case]-1)) {
      ...
      if(!any(is.na(y[i,]))) like <- like + dnorm(y[i,],onestep[i],sqrt(1/H[i]),log=TRUE)

      if(!any(is.na(y[i,]))) w[i+1,] <- A%*%K%*%y[i,] + t_L%*%as.matrix(w[i,]) else w[i+1,] <- A%*%w[i,]
      if(!any(is.na(y[i,]))) P[i+1,] <- as.vector(t_L%*%t_P%*%t(A) + Q) else P[i+1,] <- A%*%t_P%*%t(A) + Q
    ...
    t_P <- matrix(P[et[case],],ncol=nx)
    H[et[case]] <- 1/(B%*%t_P%*%t(B) + R)
    K <- t_P%*%t(B)%*%H[et[case]]
    L[et[case],] <- t_L <- A%*%(II - K%*%B)
...
dlrm.smoother <- function(y,x,A,ws,Sigma,w,P,H,L,nt=nrow(x),nx=ncol(x),lt=1,bt=1,et=nt) {
  ...
  for(case in 1:lt) {
    ...
    for(i in et[case]:bt[case]) {
      ...
      if(!any(is.na(y[i,]))) u <- as.vector(as.matrix(x[i,])%*%H[i]%*%(y[i,] - matrix(x[i,],ncol=nx)%*%w[i,]) + t(t_L)%*%u) else u <- t(A)%*%u
      if(!any(is.na(y[i,]))) U  <- as.matrix(x[i,])%*%H[i]%*%matrix(x[i,],ncol=nx) + t(t_L)%*%U%*%t_L else U <- t(A)%*%U%*%A
      wks[i,] <- w[i,] + t_P%*%u
...
#  Sigma <- apply(smth$P0s,c(1,2),mean)
  ...
  A <- b%*%solve(a) # A = Phi
  if(sum(is.na(y)) == 0) R <-(1/nt)*sum(p^2 + colSums(matrix((as.vector(apply(x,1,rep,times=nx))*as.vector(apply(x,1,rep,each=nx))),nrow=ncol(smth$Pks))*t(smth$Pks))) else {
    nna <- !is.na(y)
    nt <- sum(nna)
    p <- p[nna,]
    x <- x[nna,]
